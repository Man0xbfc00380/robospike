...
MSG_TYPE_TO_CPP = {
    ...
    'wstring': 'std::basic_string<char16_t, std::char_traits<char16_t>, ' +
                'typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<char16_t> >',
...
def msg_type_to_cpp(type_):
    ...
    if isinstance(type_, AbstractNestedType):
        if isinstance(type_, UnboundedSequence):
            return \
                ('std::vector<%s, typename ' + 
                 'std::allocator_traits<ContainerAllocator>::template rebind_alloc<%s> >') % (cpp_type, cpp_type)
                # ('std::vector<%s, typename ContainerAllocator::template rebind<%s>::other>') % (cpp_type, cpp_type)
                
                
        elif isinstance(type_, BoundedSequence):
            return \
                ('rosidl_generator_cpp::BoundedVector<%s, %u, typename ' + 
                 'std::allocator_traits<ContainerAllocator>::template rebind_alloc<%s> >') % (cpp_type, type_.maximum_size, cpp_type)
                # ('rosidl_generator_cpp::BoundedVector<%s, %u, typename ContainerAllocator::template rebind<%s>::other>') % (cpp_type, type_.maximum_size, cpp_type)
