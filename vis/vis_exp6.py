...
# Set Configs
case_name_a = "exp1/co_t_mexe_t_light"
case_name_b = "exp1/co_t_mexe_f_light"
case_name_c = "exp1/co_f_mexe_f_light"
...
for t in range(len(timer_a)):
    for i in range(len(cb_a[t])):
        for j in range(len(cb_a[t][i])):
            if cb_a[t][i][j] < timer_a[t][j]:
                print("[a]", t, i, j, len(cb_a[t][i]), len(timer_a[t]))
                cb_a[t][i][j] = 0
            else:
...
for t in range(len(timer_b)):
    for i in range(len(cb_b[t])):
        for j in range(len(cb_b[t][i])):
            if cb_b[t][i][j] < timer_b[t][j]:
                print("[b]", t, i, j, len(cb_b[t][i]), len(timer_b[t]))
                cb_b[t][i][j] = 0
            else:
...
for t in range(len(timer_c)):
    for i in range(len(cb_c[t])):
        for j in range(len(cb_c[t][i])):
            if cb_c[t][i][j] < timer_c[t][j]:
                print("[c]", t, i, j, len(cb_c[t][i]), len(timer_c[t]))
                cb_c[t][i][j] = 0
            else:
...
plt.savefig("./figures/ana/exp6.png", bbox_inches='tight')
